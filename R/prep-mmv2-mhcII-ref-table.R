...
# purpose: this script is to construct the MHC-II eplet reference table based on matchmaker v2.0.
# date: Match, 2021
...
# 2. in matchmaker v2.0, there are 5 eplets in Results table but not in any of Q/R/P.
#    These 5 eplets are in epregistry.com.  Listed them here just keep everything in record, they are not added to hlaR final ref table.
...
dqa <- as_tibble(t(dqa), rownames = "type", .name_repair = "minimal") %>%
...
dpa <- as_tibble(t(dpa), rownames = "type", .name_repair = "minimal") %>%
  row_to_names(row_number = 1) %>%
  mutate(allele = ifelse(str_detect(allele, "av"), "AbV", "oth")) %>%
  dplyr::rename(type = allele) %>%
  group_by(type) %>%
...
dqb <- as_tibble(t(dqb), rownames = "type", .name_repair = "minimal") %>%
  row_to_names(row_number = 1) %>%
  mutate(allele = ifelse(str_detect(allele, "av"), "AbV", "oth")) %>%
  dplyr::rename(type = allele) %>%
  group_by(type) %>%
...
dpb <- as_tibble(t(dpb), rownames = "type", .name_repair = "minimal") %>%
  row_to_names(row_number = 1) %>%
  mutate(allele = ifelse(str_detect(allele, "av"), "AbV", "oth")) %>%
  dplyr::rename(type = allele) %>%
  group_by(type) %>%
...
# r
r <- as_tibble(t(r), rownames = "type", .name_repair = "minimal") %>%
  row_to_names(row_number = 1) %>%
  mutate(allele = ifelse(str_detect(allele, "av"), "AbV", "oth")) %>%
  dplyr::rename(type = allele) %>%
  group_by(type) %>%
