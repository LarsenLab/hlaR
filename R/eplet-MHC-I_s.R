#' @name CalEpletMHCI_s
#' @title calculate HLA Class I eplet mismatch using MatchMaker reference table and algorithm, single molecule level
#' @param dat_in
...
#' re2 <- CalEpletMHCI_s(dat, ver = 2)
#' re3 <- CalEpletMHCI_(dat, ver = 3)
#' }
#'
...
CalEpletMHCI_s <- function(dat_in, ver = 3) {

  #* step 1: raw eplet table *#
  if(ver == 2){
  ...
  dat <- left_join(tmp_rcpt, tmp_don, by = c("part_id")) %>%
  ...
  dat_ep <- tbl_raw_eplet %>%
            select(index, type)

  #* step 4: pull out eplet of each allele *#
  for (i in 1:subj_num) {
    allele <- toupper(unlist(transpose(dat[i,-c(1)]), use.names = F))
