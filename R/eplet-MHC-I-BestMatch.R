...
CalEpletMHCI_best <- function(dat_in, id_don, id_rcpt) {
  ...
  don <- don %>% select(-c(participant_type,donor_type)) %>% setNames(c("don_id", nm_don)) # %>% mutate(newid = row_number())
  rcpt <- rcpt %>% select(-c(participant_type,donor_type)) %>% setNames(c("rcpt_id", nm_rec)) # %>% mutate(newid = row_number())

  dat <- crossing(rcpt, don) %>% select(rcpt_id, don_id, contains("rec_"), contains("don_"))
