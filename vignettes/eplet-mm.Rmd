...
single report: mis-match eplets and count for each allele
overall report: total mis-match of each subject
```{r, message = FALSE}
# use the testing data in the library
dat <-  read.csv(system.file("extdata/example", "MHC_I_test.csv", package = "hlaR"), sep = ",", header = TRUE) 
re <- CalEpletMHCI(dat)
report_single <- re$single_detail
report_overall <- re$overall_count
...
# re <- CalEpletMHCII_s(dat)
# head(re$count)
# head(re$detail)
# 
# path_test <- system.file("extdata/example", "MHC_I_test.csv", package="hlaR")
...
# out <- CalEpletMHCI_s(high_res_cohort)
```
